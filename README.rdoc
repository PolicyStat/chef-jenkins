...
* jenkins[:server][:java_args] - Java arguments given to the jenkins Java process
...
Installs a Jenkins CI server using the http://jenkins-ci.org/redhat RPM or Debian/Ubuntu package.  The recipe also generates an ssh private key and stores the ssh public key in the node 'jenkins[:pubkey]' attribute for use by the node recipes.
...
Refer to your nginx cookbook for further options, but you might want to tweak `node[:nginx][:install_method]` and you might need to set `node[:nginx][:default_site_enabled]` to false.
